...
public class DubSub extends LinearOpMode {
    // 5 specimen
    Hobbes hob = null;
    PinpointDrive drive;
    ...
    public void runOpMode() {
        ...
        while (opModeInInit() && gamepadding) {
            ...
            if (state == 0) { telemetry.addLine("First Sample:\n"); telemetry.addData("Y Coord", y); telemetry.addData("X Coord", x); telemetry.addData("Angle", swiv); telemetry.update(); }
            if (state == 1) { telemetry.addLine("Second Sample:\n"); telemetry.addData("Y Coord", y2); telemetry.addData("X Coord", x2); telemetry.addData("Angle", swiv2); telemetry.update(); }
            if (state == 2) {telemetry.addLine("All set!"); telemetry.update(); gamepadding = false;}
            ...
            telemetry.addLine("Left bumper to accept");
            telemetry.addLine("Right bumper to go back");
            telemetry.update();

            lastGamepad1.copy(gamepad1);
             if (gamepad1.left_bumper) state++;
            if (gamepad1.right_bumper) state--;
        }
        swiv = 0.05/15 * swiv;
        swiv2 = 0.05/15 * swiv2;
        ...
        Actions.runBlocking(
                new ParallelAction(
                        new SequentialAction(
                                ...
                                //hob.actionWait(1000), // tune
                                hob.actionMacro(new HobbesState(EXTENDO_OUT_FULL,
                                        EXTENDO_ARM_ABOVE_PICKUP, EXTENDO_WRIST_PICKUP,
                                        null, null, null,
                                        null, null, null, 0.3+swiv2, null)),
                                hob.actionWait(500),
                                hob.actionMacro(EXTENDO_CLAW_BEFORE_PICKUP), // CHANGE MACRO FOR SWIVEL
                                hob.actionWait(200), // tune
